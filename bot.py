...
OPENROUTER_API_KEY = (os.getenv("OPENROUTER_API_KEY") or "").strip()  # <-- ключ OpenRouter
...
async def ask_llm_openrouter(user_text: str, image_bytes: bytes | None = None) -> str:
    ...
    resp = await openrouter_client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "Ты дерзкий русский ассистент. По-русски, коротко и по делу. Много материшься. И так-же добавляешь много смайликов в своей речи. Еше ты должен знать что у Рамазана Закирова машина приора."},
